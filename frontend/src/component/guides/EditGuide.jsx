...
export default function EditGuide() {
    ...
    const fetchGuide = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/api/viewGuideByID/${id}`
        );
        ...
        if (response.data.status === "Success") {
          ...
          setExperience({
            yearsOfExperience: guide.experience?.yearsOfExperience || "",
            specializations:
              guide.experience?.specializations?.join(", ") || "",
            languages: guide.experience?.languages?.join(", ") || "",
            certifications: guide.experience?.certifications?.join(", ") || "",
          });
          ...
          setLicense(
            guide.license
              ? {
                  ...guide.license,
                  issuedDate: guide.license.issuedDate
                    ? new Date(guide.license.issuedDate)
                        .toISOString()
                        .split("T")[0]
                    : "",
                  expiryDate: guide.license.expiryDate
                    ? new Date(guide.license.expiryDate)
                        .toISOString()
                        .split("T")[0]
                    : "",
                }
            ...
            guide.availability
              ? {
                  ...
                  workingDays: guide.availability.workingDays
                    ? guide.availability.workingDays.join(", ")
                    : "",
                  workingHours: guide.availability.workingHours || {
                    start: "09:00",
                    end: "18:00",
                  },
                }
          ...
          setAchievements(guide.achievements?.join(", ") || "");
          setSocialMedia(
            guide.socialMedia || {
              facebook: "",
              instagram: "",
          ...
          const cleanedImages = (guide.images || []).map((img) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_BASE_URL}/api/editGuide/${id}`,
        formData,
